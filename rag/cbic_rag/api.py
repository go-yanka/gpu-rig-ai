...
# 2026-04-25: corpus-grounded refusal signal (replaces topic classifier).
try:
    from groundedness import check_groundedness as _check_groundedness
except Exception:
    _check_groundedness = None
...
# 2026-04-26: NO-PROXY restriction (codified user rule). Groundedness path
# bypasses the LiteLLM gateway and calls qwen3-14b llama-server directly.
# Default points to local llama-server on GPU 2 :9082 (qwen3-14b service).
# To re-enable proxy mode: set GROUNDED_LLM_URL=$LITELLM_URL.
GROUNDED_LLM_URL   = os.environ.get('GROUNDED_LLM_URL',   'http://127.0.0.1:9082')
GROUNDED_LLM_MODEL = os.environ.get('GROUNDED_LLM_MODEL', 'qwen3-14b-q4_k_m.gguf')
GROUNDED_LLM_KEY   = os.environ.get('GROUNDED_LLM_KEY',   'sk-anything')
...
class QueryReq(BaseModel):
    ...
    # 2026-04-25: when True, /retrieve runs groundedness check on top-K
    # and returns {grounded: yes|partial|no, grounded_reason: ...}.
    grounded: Optional[bool] = False


def _call_llm_short(system: str, user: str, *, temperature: float = 0.0,
                    max_tokens: int = 60) -> str:
    """Groundedness LLM call. Direct to llama-server :9082 — NO PROXY (codified
    user rule 2026-04-26). Caller-controlled max_tokens kept short (~60) so
    groundedness verdicts complete in ~3s on qwen3-14b single-slot."""
    with httpx.Client(timeout=60) as cli:
        r = cli.post(f'{GROUNDED_LLM_URL}/v1/chat/completions',
                     headers={'Authorization': f'Bearer {GROUNDED_LLM_KEY}',
                              'Content-Type': 'application/json'},
                     json={'model': GROUNDED_LLM_MODEL,
...
def _call_llm(system: str, user: str, temperature: float = 0.0) -> str:
    """Primary LLM call (used by /query answer + HyDE + router).

    2026-04-26: NO-PROXY compliance — defaults to direct llama-server :9082
    (qwen3-14b service) instead of LiteLLM gateway. Override with
    LLM_DIRECT_URL / LLM_DIRECT_MODEL / LLM_DIRECT_KEY envs. To re-enable
    proxy mode set LLM_DIRECT_URL=$LITELLM_URL and LLM_DIRECT_MODEL=qwen3-14b-hermes.
    """
    url   = os.environ.get('LLM_DIRECT_URL',   GROUNDED_LLM_URL)   # default direct :9082
    model = os.environ.get('LLM_DIRECT_MODEL', GROUNDED_LLM_MODEL) # default qwen3-14b-q4_k_m.gguf
    key   = os.environ.get('LLM_DIRECT_KEY',   GROUNDED_LLM_KEY)
    with httpx.Client(timeout=120) as cli:
        r = cli.post(f'{url}/v1/chat/completions',
...
def retrieve_only(req: QueryReq):
    """Retrieval + rerank only (no LLM). For gate evaluation speed.

    Skips LLM call (~13s/query saving). Returns reranked top-k as hits
    with full payload so G1/G3 evaluators can score recall and citations.
    """
    import time as _t
    t0 = _t.perf_counter()
    filters = req.filters
    ...
    if USE_ROUTER and not filters:
        ...
        if router_cat:
            filters = filters_for(router_cat)
    search_text = hyde(req.question) if USE_HYDE else req.question
    hits = retrieve(search_text, k=TOP_K_RETR, filters=filters, collection=req.collection)
    if not hits and filters:
        hits = retrieve(search_text, k=TOP_K_RETR, filters=None, collection=req.collection)
    if not hits:
        return {'hits': [], 'router_category': router_cat,
                'timings': {'total_ms': round((_t.perf_counter()-t0)*1000.0,2)}}
    if RERANK == 'colbert':
        top = rerank_colbert(req.question, hits, top_n=req.k)
    elif RERANK == 'ce':
        top = rerank(req.question, hits, top_n=req.k)
    ...
    flat = []
    for h in top:
        p = h if isinstance(h, dict) else {}
        flat.append({'chunk_id': p.get('chunk_id') or p.get('id'),
                     'doc_id': p.get('doc_id'),
                     'linked_doc_ids': p.get('linked_doc_ids') or [],  # Defect D: shared-PDF
                     'section_ref': p.get('section_ref'),
                     'title': p.get('title'),
                     'text': p.get('text') or p.get('excerpt'),
                     'score': p.get('score'),
                     'rerank_score': p.get('rerank_score')})
    grounded_verdict = None
    if getattr(req, 'grounded', False) and _check_groundedness is not None:
        gt0 = _t.perf_counter()
        try:
            grounded_verdict = _check_groundedness(
                req.question, flat,
                llm_caller=lambda sys_, usr_, temperature=0.0, max_tokens=60:
                    _call_llm_short(sys_, usr_, temperature=temperature, max_tokens=max_tokens))
            grounded_verdict['ms'] = round((_t.perf_counter()-gt0)*1000.0, 2)
        except Exception as e:
            grounded_verdict = {'grounded': 'unknown', 'reason': f'exc:{type(e).__name__}'}
    out = {'hits': flat, 'router_category': router_cat,
           'timings': {'total_ms': round((_t.perf_counter()-t0)*1000.0,2)}}
    if grounded_verdict is not None:
        out['grounded'] = grounded_verdict.get('grounded')
        out['grounded_reason'] = grounded_verdict.get('reason')
        out['grounded_ms'] = grounded_verdict.get('ms')
    return out
