#!/usr/bin/env python3
"""
Indian Legal AI — RAG API with Chat UI
Port: 7000

  GET  /          → Chat interface (browser UI)
...
Use ONLY the provided legal context. Always cite the specific Act and Section number.
If the context does not fully cover the question, say so and advise consulting a lawyer.
...
try:
    ...
    print(f"Index loaded: {len(documents)} docs", flush=True)
...
app = FastAPI(title="Indian Legal AI", version="1.0-poc")
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Header ── */
  .header {
    background: linear-gradient(135deg, #1a1f2e 0%, #141928 100%);
...
<div class="header">
  ...
  <div class="header-text">
    ...
    <p>Income Tax · GST · IPC · Constitution · FEMA · Companies Act · NI Act</p>
  </div>
  <span class="badge" id="status-badge">● RAG Ready</span>
</div>
...
<div class="chat-area" id="chat">
  <div class="welcome" id="welcome">
    ...
    <div class="suggestions">
      <button class="suggestion-btn" onclick="ask('What is the maximum deduction under Section 80C?')">💰 What is the 80C deduction limit?</button>
      <button class="suggestion-btn" onclick="ask('Can I claim HRA and home loan interest deduction together?')">🏠 HRA + home loan deduction together?</button>
...
<div class="input-bar">
  <div class="input-row">
    ...
    <button class="send-btn" id="send-btn" onclick="sendQuestion()" title="Send">➤</button>
  </div>
...
function formatAnswer(text) {
  // Basic formatting: bold, bullet points, section numbers highlighted
  return text
...
async def ask(q: Question):
    ...
    if not hits:
        return {
            "answer":         "No relevant legal sections found in the knowledge base.",
            "sources":        [],
            "context_chunks": 0
        }
