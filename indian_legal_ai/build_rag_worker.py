#!/usr/bin/env python3
"""
Parallel index-builder worker.

Processes a list of curated JSONL files through one BGE endpoint (one GPU)
and upserts into the shared Qdrant collection 'indian_legal_full'.

Invocation:
  python build_rag_worker.py \
      --endpoint http://localhost:9090/v1/embeddings \
      --id-base 10000000 \
      --files indian_laws,ipc_sections

Each worker uses a disjoint id-base range so points never collide.
Whole-file assignment means no overlap with other workers.
"""

import argparse, os, json, sys, time, datetime, gc, urllib.request
...
def main():
    ...
    wid = args.worker_id
    log(wid, f"endpoint={args.endpoint}  id-base={args.id_base:,}  files={args.files}")
