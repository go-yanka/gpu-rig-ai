...
EMBED_GPUS_DEFAULT = "4,5,6"
EMBED_GPUS_FORBIDDEN = {"0", "1", "2", "3"}
...
TEXT_DENSITY_MIN = 200  # chars/page threshold — below this we call it OCR

def detect_text_source(full_text: str, page_offsets: list[int]) -> str:
    n_pages = max(1, len(page_offsets) - 1)
    density = len(full_text.strip()) / n_pages
    return "ocr" if density < TEXT_DENSITY_MIN else "born"
...
def phase2(limit=None, resume=True, use_qwen_first=False, doc_ids=None):
    ...
    for i, r in enumerate(docs):
        ...
        if not path or not os.path.exists(path):
            reason = f"missing_path: path_en={path_en!r} path_hi={path_hi!r} exists={os.path.exists(path) if path else False}"
...
def _insert_chunk(c2, ch: Chunk, dup_of: str | None):
    ...
    c2.execute("""INSERT OR REPLACE INTO chunks
                  ...
                  VALUES (?,?,?,?,?)""",
               (ch.chunk_id, ch.doc_id, json.dumps(payload),
                0 if dup_of else 1, dup_of))
...
def _flush_batch(c2, qc, buf, embed_batch, upsert_chunks):
    """Embed + upsert a batch. RAISES on failure (do NOT swallow) — the previous
    print-and-continue behaviour let phase3-5 print "DONE" with 0 real upserts
    (smoke exposed this 2026-04-23). If embed or upsert fails the batch is
    re-raised so phase3_4_5 exits non-zero and the screen session dies visibly."""
    texts = [p.get("embed_text") or p.get("text") for p in buf]
    dense, sparse = embed_batch(texts)          # raises on embed failure
    n = upsert_chunks(qc, buf, dense, sparse)   # raises on Qdrant failure
...
def main():
    ...
    ap.add_argument("--phase", choices=["phase1", "phase2", "phase3_4_5", "all"], default="all")
    ...
    needs_preflight = (args.phase in ("phase2", "phase3_4_5", "all")) and not args.no_preflight
    if needs_preflight:
        run_preflight()
