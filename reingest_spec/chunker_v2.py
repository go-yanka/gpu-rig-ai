...
def _tolerant_json_loads(raw: str) -> dict:
    ...
    if start != -1:
        ...
        if end != -1:
            ...
            # L4 (added 2026-04-25): UNCLOSED — qwen3 truncated mid-JSON. Best-effort
            # completion: close the open brackets in reverse order, drop any
            # dangling key:partial-value at the tail, and retry. Recovers
            # truncated 'schedule'/'tabular' plans whose hard_boundaries regex
            # array overflowed max_tokens. Handles mid-string truncation by
            # rewinding to the last safe comma boundary BEFORE the open string.
            tail = s[start:]
            stack = []
            in_str = False
            esc = False
            last_safe = -1   # last comma at depth>0, outside any string
            string_start = -1  # offset where current open string started
            for i, ch in enumerate(tail):
                ...
                if ch == "," and not in_str:
                    last_safe = i
            if stack and last_safe > 0:
                # Cut at last_safe (drops any dangling key:partial-value or open string)
                truncated = tail[:last_safe]
                closes = "".join("}" if c == "{" else "]" for c in reversed(stack))
                candidate = truncated + closes
                candidate = re.sub(r",(\s*[}\]])", r"\1", candidate)
                try:
                    return json.loads(candidate)
                except json.JSONDecodeError:
                    pass
    # L5: give up with the ORIGINAL error so the caller's log names the real
...
_DEFAULT_PLANS_BY_PREFIX = {
    ...
    "cbic-circular-msts": dict(
        ...
        has_amendments=False,
        hierarchy_depth=1,
        notes="default plan: circular (Defect C fallback)",
    ),
    "cbic-notification-msts": dict(
        doc_type="notification",
        structure="flat_paragraphs",
        primary_splitter="paragraph",
        critical_units=["proviso", "explanation", "schedule", "amendment"],
        hard_boundaries=[],
        table_regions=[],
        has_amendments=False,
        hierarchy_depth=1,
        notes="default plan: notification (Defect C fallback)",
    ),
    "cbic-rule-msts": dict(
        doc_type="rules",
        structure="hierarchical_sections",
        primary_splitter="rule",
        critical_units=["proviso", "explanation", "definition"],
        hard_boundaries=[],
        table_regions=[],
        has_amendments=False,
        hierarchy_depth=2,
        notes="default plan: rule (Defect C fallback)",
    ),
    "cbic-order-msts": dict(
        doc_type="order",
        structure="flat_paragraphs",
        primary_splitter="paragraph",
        critical_units=["proviso", "explanation"],
        hard_boundaries=[],
        table_regions=[],
        has_amendments=False,
        hierarchy_depth=1,
        notes="default plan: order (Defect C fallback)",
    ),
    "cbic-regulation-doc-msts": dict(
        doc_type="regulation",
        structure="hierarchical_sections",
        primary_splitter="section",
        critical_units=["proviso", "explanation", "definition"],
        hard_boundaries=[],
        table_regions=[],
        has_amendments=False,
        hierarchy_depth=2,
        notes="default plan: regulation (Defect C fallback)",
    ),
    "cbic-others-document-msts": dict(
        doc_type="other",
        structure="flat_paragraphs",
        primary_splitter="paragraph",
        critical_units=[],
        ...
        has_amendments=False,
        hierarchy_depth=1,
        notes="default plan: others (Defect C fallback)",
    ),
    "cbic-allied-act-dtls": dict(
        doc_type="act",
        structure="hierarchical_sections",
        primary_splitter="section",
        critical_units=["proviso", "explanation", "definition"],
        ...
        has_amendments=False,
        hierarchy_depth=2,
        notes="default plan: act (Defect C fallback)",
    ),
    # Generic catch-all for any unknown prefix — uses paragraph splitter
    # which is the safe default. Only applies when no prefix-specific
    # template exists, as a last-resort rescue.
    "_GENERIC_": dict(
        doc_type="mixed",
        structure="flat_paragraphs",
        primary_splitter="paragraph",
        ...
        has_amendments=False,
        hierarchy_depth=1,
        notes="default plan: generic (Defect C fallback)",
    ),
}

# Prefixes that ALWAYS bypass qwen3 (deterministic, no classifier needed)
_BYPASS_QWEN_PREFIXES = (
    "cbic-form-msts", "cbic-instruction-msts", "cbic-attachment-dtls",
    "cbic-circular-msts", "cbic-notification-msts", "cbic-rule-msts",
    "cbic-order-msts", "cbic-regulation-doc-msts",
    "cbic-others-document-msts", "cbic-allied-act-dtls",
)
...
def _default_plan_for(meta: dict) -> "ChunkingPlan | None":
    ...
    tmpl = _DEFAULT_PLANS_BY_PREFIX.get(prefix) or _DEFAULT_PLANS_BY_PREFIX.get("_GENERIC_")
    if tmpl is None:
        return None
...
def classify_doc_qwen(meta: dict, head: str, tail: str, toc: str = "", page_map: list = None,
    ...
    for attempt in range(_retries):
        ...
        # 2026-04-25: bumped 1024 → 2048. cbic-notification-msts:1000998 is a
        # 'schedule' / 'tabular' rate-notification whose hard_boundaries regex
        # array overflows 1024 (markdown-fenced, escaped backslashes inflate
        # token count). 2048 covers all observed schedule/tabular plans plus
        # _tolerant_json_loads L4 (best-effort brace completion) gives a safety
        # net for any remaining truncation.
        resp = requests.post(QWEN3_URL, json={
            "prompt": body, "max_tokens": 512, "temperature": 0.0, "stop": ["\n\n\n"],
        }, timeout=timeout)
        ...
        try:
            ...
            last_err = e
            continue
    # Defect C(b): exhausted retries — try prefix-based default plan as
...
# 2026-04-24 — SEMANTIC-BOUNDARY markers for Indian legal drafting style.
# When a Section is too long for one chunk, prefer splitting at these
# self-contained semantic units before falling back to generic (1)/(a)
# subsection numbering or sentence boundaries. Each match here marks the
# *start* of a new semantic unit. Generic across CGST/IGST/UTGST/Customs/
# Income Tax/Allied Acts and any rules/notifications that follow Indian
# legal drafting conventions.
#
# Why we needed this (Defect C, 2026-04-24): CGST §17 mean-pooled into one
# 3551-char chunk diluted the P&M Explanation to ~7% signal mass; queries
# about "structural supports / plant and machinery" lost to HSN tariff
# notifications. Splitting at "Explanation .-" gives the definition its
# own embedding pool. Same pattern applies corpus-wide.
SEMANTIC_BOUNDARY_RE = re.compile(
    r"(?m)^"                                            # line start
    r"(?:"
    r"\s*\*?\s*Explanation\s*\d*\s*\.\s*\-"            # Explanation .-, Explanation 2.-, * Explanation .-
    r"|\s*Provided\s+(?:further\s+|also\s+)?that\b"     # Provided that / Provided further that / Provided also that
    r"|\s*\*\s*Enforced\s+w\.e\.f"                      # * Enforced w.e.f. (start of footnote block)
    r"|\s*\d+\.\s+(?:Inserted|Substituted|Omitted|Added|Renumbered)\s+(?:for\s+|by\s+)"  # footnote: 1. Inserted by / 2. Substituted for
    r"|\s*Illustration\s*\d*\s*\.\s*\-"                 # Illustrations
    r")"
)
...
def chunk_document(full_text: str, plan: ChunkingPlan, meta: dict,
    ...
    # -- R9 (2026-05-08) HARD CEILING ENFORCEMENT --------------------------------
    # Audit at 03:30 IST found 25 chunks > 8000 chars (max 438KB) — narrative
    # text was slipping past section/prose splitters because some plans (allied_acts,
    # large notifications) returned no section markers and the prose splitter's
    # iterative loop didn't always force a split at CEILING. One 438KB chunk = one
    # embedding vector for an entire Act. Retrieval cannot find specific clauses.
    # This pass guarantees no chunk exceeds CEILING by splitting on paragraph,
    # then sentence, then char boundary as last resort.
    all_chunks = _enforce_ceiling(all_chunks, plan, meta)
...
def _extract_table_chunks(text: str, plan: ChunkingPlan, meta: dict,
    ...
    # 2026-04-25 nullguard: qwen3 sometimes omits table_regions from plan JSON
    # (especially on table_row primary_splitter where it considers the whole
    # doc table-like). Default-empty so iteration is safe. Same pattern as
    # `(plan.hard_boundaries or [])` at line 833.
    for tr in (plan.table_regions or []):
...
def _chunk_prose_span(span: str, span_offset: int, plan: ChunkingPlan, meta: dict,
    ...
    while cur < N:
        ...
        # 2026-04-26 (Defect F-tail-dup): if this chunk consumed the entire
        # span, a further iteration starting at (adj_end - overlap) would
        # emit a chunk whose char range is 100% inside the previous chunk's
        # range — i.e. a duplicate tail with overlap-window text only.
        # Observed on cbic-notification-msts:1008760 (Set 5 G3 miss):
        # chunk[7662:8699] followed by chunk[7999:8699] both R5:mid_700.
        # Break here — no new text remains to chunk.
        if adj_end >= N:
            break
...
def _section_bounded_split(span: str, span_offset: int, plan: ChunkingPlan,
    ...
    for i, (sec_off, header_line, label) in enumerate(starts):
        ...
        # -- single-chunk section (small enough to fit one TARGET-sized chunk) --
        # 2026-04-26 (Defect F-megachunk): previously this was `<= CEILING (8000)`,
        # which let any section up to 8K chars become one chunk. Observed on
        # cbic-notification-msts:1005850 (Set 5 G3 miss): a 7,087-char anti-
        # dumping notification body emitted as one chunk, diluting the rerank
        # signal so the right doc lost to a topically-adjacent doc on a
        # value-addition / anti-circumvention query.
        # New threshold: 1.5 × TARGET (5,250 chars) for one-chunk emission;
        # larger sections fall through to the sub-split path below which
        # uses semantic + subnumeric markers ((a)/(b), Explanation, Provided).
        _SINGLE_SECTION_LIMIT = int(TARGET * 1.5)
        if len(body) <= _SINGLE_SECTION_LIMIT:
            chunks.append(_mk_section_chunk(
        ...
        # 2026-04-24: include SEMANTIC_BOUNDARY_RE markers (Explanation,
        # Provided that, footnote starts, Illustration). These are
        # *preferred* boundaries — they isolate self-contained semantic
        # units like the §17 P&M Explanation, so dense pooling captures
        # each definition cleanly instead of diluting across the section.
        semantic_pts = {
            sec_off + m.start()
            for m in SEMANTIC_BOUNDARY_RE.finditer(body)
        }
        subnum_pts = {
            sec_off + m.start()
        ...
        sub_pts = sorted({sec_off} | semantic_pts | subnum_pts | {sec_end})
        sem_sorted = sorted(semantic_pts)
        sub_sorted = sorted(subnum_pts)
        # walk pairs, packing into [TARGET..CAP] windows aligned to sub_pts
        # 2026-04-24 priority order:
        #   1. EARLIEST semantic_pt in [cur_sub+FLOOR, cap] — Explanations,
        #      Provisos, footnote tails should each START a new chunk so
        #      dense pooling captures them as standalone units.
        #   2. LATEST subnum_pt in [cur_sub+FLOOR, target] — pack subsections.
        #   3. LATEST subnum_pt in [cur_sub+FLOOR, cap] — overflow.
        ...
        while cur_sub < sec_end:
            ...
            best = None
            # (1) earliest semantic boundary in [FLOOR..cap]
            for p in sem_sorted:
                if cur_sub + FLOOR <= p <= cap:
                    best = p
                    break
            # (2,3) latest sub-numbering boundary
            if best is None:
                for p in sub_sorted:
...
def _mk_section_chunk(body_text: str, start: int, end: int,
    ...
    if section_header and not body_text.lstrip().startswith(section_header):
        ...
        text = body_text
    # 2026-04-24 Contextual prefix (Gemini r1 lever, generic corpus-wide):
    # prepend a structural breadcrumb to embed_text ONLY (not visible text)
    # so BGE-M3 attention anchors orphaned clauses to their statutory home.
    # Form: "Doc: {category}/{subcategory} | {doc_type} | Section: {label} | "
    cat = meta.get("category", "") or ""
    subcat = meta.get("subcategory", "") or ""
    doc_type = (meta.get("doc_type") or "").strip()
    breadcrumb_parts = []
    if cat:
        breadcrumb_parts.append(f"Doc: {cat}" + (f"/{subcat}" if subcat else ""))
    if doc_type:
        breadcrumb_parts.append(f"Type: {doc_type}")
    if section_label:
        breadcrumb_parts.append(f"Section: {section_label}")
    breadcrumb = " | ".join(breadcrumb_parts)
    base = (parent + "\n\n" + text) if parent else text
    embed_text = (breadcrumb + "\n" + base) if breadcrumb else base
    return Chunk(
        chunk_id=sha256_of(text),
        doc_id=meta.get("doc_id", meta.get("source", "unknown")),
        sha256=sha256_of(text),
...
def _enforce_ceiling(chunks: list[Chunk], plan: ChunkingPlan, meta: dict) -> list[Chunk]:
    """R9 (2026-05-08): split any chunk whose text exceeds CEILING.

    Cascade:
      1. Try paragraph boundaries (\\n\\n) — keep chunks ~TARGET-sized.
      2. Try sentence boundaries (. + space + capital).
      3. Hard char-position split with 200-char overlap.

    Each split chunk inherits the parent's payload (section_ref, parent_hierarchy_text,
    chunk_type, etc.) and gets a `R9:ceiling_split` rule tag.
    """
    out: list[Chunk] = []
    for c in chunks:
        if len(c.text) <= CEILING:
            out.append(c)
            continue
        # Need to split this chunk. Try paragraph boundaries first.
        text = c.text
        pieces: list[str] = []
        # Strategy 1: paragraph split, accumulate up to TARGET
        paras = re.split(r"\n\n+", text)
        if len(paras) > 1:
            buf = ""
            for para in paras:
            ...
            if buf:
                pieces.append(buf)
            # Validate: any piece still > CEILING? Fall through to sentence split.
            if any(len(p) > CEILING for p in pieces):
                pieces = []
        # Strategy 2: sentence split (only used if paragraph strategy failed)
        if not pieces:
            ...
            if len(sents) > 1:
                ...
                if any(len(p) > CEILING for p in pieces):
                    pieces = []
        # Strategy 3: hard char split with overlap
        if not pieces:
            STEP = TARGET
            OVERLAP = 200
            i = 0
            while i < len(text):
                ...
                if end >= len(text):
                    break
                i = end - OVERLAP
        # Emit pieces as new chunks inheriting parent payload (use page_range tuple offsets).
        n_pieces = len(pieces)
        parent_start, parent_end = c.page_range
        char_off = parent_start
        for idx, piece in enumerate(pieces):
            piece_start = char_off
            piece_end = char_off + len(piece)
            new_id = hashlib.sha256(piece.encode("utf-8", errors="ignore")).hexdigest()
            new = Chunk(
                chunk_id=new_id,
                doc_id=c.doc_id,
                sha256=new_id,
                ...
                lang=c.lang,
                text=piece,
                embed_text=(c.parent_hierarchy_text + "\n\n" + piece) if c.parent_hierarchy_text else piece,
                section_ref=c.section_ref,
                parent_hierarchy_text=c.parent_hierarchy_text,
                chunk_type=c.chunk_type,
                is_table=c.is_table,
                table_part=c.table_part,
                page_range=(piece_start, piece_end),
                effective_date=c.effective_date,
                text_source=c.text_source,
                hindi_twin_chunk_ids=list(c.hindi_twin_chunk_ids),
                topic_tags=list(c.topic_tags),
                also_appears_in=list(c.also_appears_in),
                dup_of_chunk_id=c.dup_of_chunk_id,
                chunking_plan_used=c.chunking_plan_used,
                chunking_rule_triggered=list(c.chunking_rule_triggered) + [f"R9:ceiling_split[{idx+1}/{n_pieces}]"],
                notification_id=c.notification_id,
                as_of_date=c.as_of_date,
                superseded_by=c.superseded_by,
            )
            out.append(new)
            # Advance offset accounting for overlap (200 chars when hard char-split was used).
            char_off = piece_end - (200 if idx < n_pieces - 1 and any("char_split" not in str(p) for p in [""]) else 0)
    return out
...
def classify_and_chunk(full_text: str, meta: dict,
                       ...
                       page_offsets: list[int] = None,
                       page_map: list = None,
                       use_qwen_first: bool = False,
                       reuse_plan: "ChunkingPlan | dict | None" = None
                       ) -> tuple[ChunkingPlan, list[Chunk]]:
    """End-to-end: Pass 1 LLM + Pass 2 rules.

    If ``reuse_plan`` is provided (either a ChunkingPlan instance or a dict
    loaded from manifest ``plan_json``), Pass 1 is SKIPPED. This saves ~40s
    per doc on re-chunk runs. Codified 2026-04-24 after GST50 re-chunk did
    43 wasted qwen3 classifies on plans that already existed in manifest.
    """
    if reuse_plan is not None:
        if isinstance(reuse_plan, dict):
            plan = ChunkingPlan(**reuse_plan)
        else:
            plan = reuse_plan
