...
def _is_hit(gold_item, hits):
    ...
    for h in hits:
        ...
        if did and did in g["docs"]:
            return True
        # Defect D fix (2026-04-25): match against linked_doc_ids for shared-PDF chunks
        ldids = payload.get("linked_doc_ids") or []
        if ldids and any(d in g["docs"] for d in ldids):
            return True
