...
# p1_v1 — import classify_query + enforce_min_non_act for statute-boost retrieval.
from retriever import retrieve, rerank, classify_query, enforce_min_non_act
...
def _run_pipeline(req: QueryReq, client_ip: Optional[str] = None) -> Dict[str, Any]:
    ...
    # p1_v1: classify query before retrieve so we can boost act/rules BM25
    # when the user explicitly names a Section / Rule / Article.
    try:
        qclass = classify_query(req.question)
    except Exception as _e:
        qclass = {'tier': 'none', 'refs': []}
    timings['retrieval_tier']       = qclass.get('tier')
    timings['statute_refs_matched'] = qclass.get('refs')
    ...
    # p1_v1: surface fusion mode used (qdrant_rrf vs client_rrf_boost_*)
    if 'fusion_mode' in _sub:
        timings['fusion_mode'] = _sub['fusion_mode']
    if 'fusion_mult' in _sub:
        timings['fusion_mult'] = _sub['fusion_mult']
    ...
    if not hits and filters:
        hits = retrieve(search_text, k=TOP_K_RETR, filters=None, qclass=qclass)
    ...
    # p1_v1: post-rerank backfill — ensure >=2 non-(act|rules) chunks in top-k
    # so the LLM still sees clarifying circulars even on 'hard' statute queries.
    try:
        top = enforce_min_non_act(top, hits, min_non_act=2)
    except Exception as _e:
        print(f'[p1_v1] enforce_min_non_act failed: {_e}')
    try:
        non_act = sum(1 for c in (top or [])
                      if str((c or {}).get('doc_type', '')).lower()
                      not in ('act', 'rules'))
        timings['non_act_count'] = non_act
    except Exception:
        timings['non_act_count'] = None
